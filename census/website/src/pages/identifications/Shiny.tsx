      ...
      >
        ...
        >
          <div className="flex-1 flex flex-col gap-2">
            <h1 className="text-4xl tracking-wide font-serif font-bold text-accent-900">
              Shiny Bugs
            </h1>
            <p className="text-balance">
              our resident entomologists have selected 24 special bugs that they want to find. this
              resets every season so get spotting and maybe you’ll unlock one and earn the{" "}
        ...
        <button
          ...
          className="text-accent-900 flex text-sm mx-auto items-center"
        >
          <SiChevronDown
            className={cn("transition-transform duration-300 text-xl", expanded && "rotate-180")}
          />
...
export const ShinyThumbnail: FC<ShinyThumbnailProps> = ({ shiny }) => {
  ...
  const hasBeenSeen = useMemo(
    () => localStorage.getItem(`shiny-unlocked-and-new:${shiny.id}`) === "true",
    [shiny],
  );
